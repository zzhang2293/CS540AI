...
if __name__ == '__main__':
    ...
    test = get_data_loader(training=False)
    criterion = nn.CrossEntropyLoss()
    train_model(model, train, criterion, 3)
    evaluate_model(model, test, criterion, show_loss=False)
    dataiter = iter(train)
    predict_set, labels = dataiter.__next__()
    predict_label(model, predict_set, 1)